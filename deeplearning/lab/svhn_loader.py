...
def load_data():
    ...
    f = gzip.open("./train_svhn.pkl.gz", "rb")
    training_data = cPickle.load(f)
    f.close()
    t = gzip.open("./test_svhn.pkl.gz", "rb")
    test_data = cPickle.load(t)
    t.close()
...
def load_data_wrapper():
    ...
    tr_d, te_d = load_data()
    training_inputs = [np.reshape(x, (3072, 1)) for x in tr_d[0]]
    training_results = [vectorized_result(y) for y in tr_d[1]]
    training_data = zip(training_inputs, training_results)
    test_inputs = [np.reshape(x, (3072, 1)) for x in te_d[0]]
